const commonUtil = () => {
     ...
     * 传入一串num四个 一个空格
     * */
    const toSplitNumFor = (num, numToSpace) => {
     ...
     * return 删除后的数组
     * */
    const deleteArrItem = (arr, arrItem) => {
     ...
     * */
    const byArrObjDeleteArrObj2 = (arrObj, arrObj2, objKey) => {
        arrObj
            .map((value) => {
                return value[objKey];
            })
     ...
     * */
    const deleteArrObjByKey = (arrObj, objKey, value) => {
        //foreach splice
     ...
     * */
    const findArrObjByKey = (arrObj, objKey, value) => {
     ...
     * return: arrObj2删除过后的数组
     * */
    const byArrObjFindArrObj2 = (arrObj, arrObj2, objKey) => {
        const arrObj3 = [];
        arrObj
            .map((value) => {
                return value[objKey];
            ...
            .forEach((value2) => {
                const arrIndex = arrObj2.findIndex(
                    (item) => item[objKey] === value2
                );
                if (arrIndex !== -1) {
    ...
    const formatAmountCurrency = (amount) => {
        ...
        const newAmountString = `RM ${newAmount}`;
